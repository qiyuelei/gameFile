...
//    menuNode->addChild(MenuBg);
    ...
    BackEquipmentBtn->setPosition(ccp(winSize.width/2,winSize.height*0.5+20));
    ...
    ResumeBtn->setPosition(ccp(winSize.width/2,winSize.height*0.5-40));
    ResumeBtn->setTag(2);
    ...
    CCLabelTTF* btnttf = CCLabelTTF::create("TAP SPEED", "Airal", 32);
   ...
   // CCMenuItemImage* pspeedBtn = CCMenuItemImage::create("pauseBtn.png", "pauseBtn.png",this,menu_selector(GameScene::setUpdateTime));
    pspeedBtn->setPosition(ccp(winSize.width*0.5, 16));
    pspeedBtn->setScale(0.5);
    pspeedBtn->setTag(1);
    ...
    pauseBtn->setPosition(ccp(winSize.width*0.5+140, winSize.height-20));
...
void GameScene::setUpdateTime(){
    ...
    if (curUpdateSpeed < 0.2) {curUpdateSpeed = 0.2;}
    this->schedule(schedule_selector(GameScene::update), curUpdateSpeed);
}
...
void GameScene::processZhadan(cocos2d::CCPoint location){
    ...
    for (int j=0; j<4; j++) {
        switch (j) {
            case 0:
                    targetPoint = CCPointMake(location.x+32, location.y);
                break;
            case 1:
                targetPoint = CCPointMake(location.x-32, location.y);
                break;
            case 2:
                targetPoint = CCPointMake(location.x, location.y+32);
                break;
            default:
                 targetPoint = CCPointMake(location.x, location.y-32);
                break;
...
void GameScene::FindPaopaoToSel(CCPoint location,int _PaopaoType){
    for (int i = 0; i<4; i++) {
        switch (i) {
            case 0:
             FindPaopaoToSel_R(CCPointMake(location.x+32, location.y), _PaopaoType);
                break;
            case 1:
                FindPaopaoToSel_R(CCPointMake(location.x, location.y-32), _PaopaoType);
                break;
            case 2:
                FindPaopaoToSel_R(CCPointMake(location.x-32, location.y), _PaopaoType);
                break;
...
void GameScene::AddMarkToPaopaoY(int ColNum){
    ...
    for (int i = 0; i< num ; i++) {
        NormalPaoPao* _myPaopao = (NormalPaoPao*)uiLayer->getMapPaopaoArray()->objectAtIndex(i);
        if (_myPaopao->getPositionX() > (32*ColNum+16) && _myPaopao->PosType == 1) {
            _myPaopao->colNum++;
        }
    }
}
...
//        }
        _myPaopao->runAction(CCSequence::create(CCMoveTo::create(0.1, CCPointMake(_myPaopao->getPositionX()-32*_myPaopao->colNum, _myPaopao->getPositionY()-32*_myPaopao->rowNum)),NULL));
        _myPaopao->rowNum = 0;
        _myPaopao->colNum = 0;
    }
    ...
    if (curBaojinStat == true) {
        ...
        CCARRAY_FOREACH(uiLayer->getChildByTag(_mSpriteBatchTag)->getChildren(), obj){
            NormalPaoPao* _myPaopao = (NormalPaoPao*)obj;
                ...
                if (_myPaopao->getPositionY() > 300) {
                    AddBaojin(_myPaopao->getPositionX());
                }
                if (_myPaopao->PosType == 1) {
                    _myPaopao->setPosition(CCPointMake(_myPaopao->getPositionX(), _myPaopao->getPositionY()+32));
                }else{
                    _myPaopao->setPosition(CCPointMake(_myPaopao->getPositionX(), _myPaopao->getPositionY()+34));
                    _myPaopao->PosType = 1;
                }
            }
    }
...
void GameScene::showBaojinPaopao(){
    ...
    for (int i = 0; i< num ; i++) {
        ...
        if (_myPaopao->baojinCol > 0) {
            ...
            CCSpawn* spawn = CCSpawn::create(CCScaleTo::create(0.05, 0.45),CCRotateTo::create(0.05, 3),NULL);
            CCSpawn* spawn1 = CCSpawn::create(CCScaleTo::create(0.05, 0.5),CCRotateTo::create(0.05, -3),NULL);
