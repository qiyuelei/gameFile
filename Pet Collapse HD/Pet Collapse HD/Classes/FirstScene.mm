...
void FirstScene::initGameOver(){
    if (isNewGame == false) {
        pMenu->setPosition(ccp(-winSize.width, 0));
  
    gameOverNode = CCNode::create();
    addChild(gameOverNode);
    
    CCSprite* gameoverBg = CCSprite::create("game_overBg.png");
    gameoverBg->setPosition(ccp(winSize.width*0.5, winSize.height*0.5));
    ...
    gameoverTitleTTF->setPosition(ccp(winSize.width*0.5, winSize.height*0.5+150));
    ...
    GoldImg->setPosition(ccp(winSize.width*0.5-54,winSize.height*0.5+50));
    ...
    curGoldTTF->setPosition(ccp(winSize.width*0.5-50,winSize.height*0.5+50));
    curGoldTTF->setScale(0.7);
    ...
    CCLabelTTF* BestscoreImgTTF = CCLabelTTF::create(languageLayer->getChar("Score:"), "Verdana-Bold", 45);
        ...
        BestscoreImgTTF->setPosition(ccp(winSize.width*0.5-54,winSize.height*0.5));
    BestscoreImgTTF->setScale(0.7);
    ...
    curScoreTTF->setPosition(ccp(winSize.width*0.5-50,winSize.height*0.5));
    curScoreTTF->setScale(0.8);
...
//        CCMenuItemSprite* okBtn = CCMenuItemSprite::create(sp1, sp2, this, menu_selector(FirstScene::moveGameOverNode));
        ...
        okBtn->setPosition(ccp(winSize.width*0.5,winSize.height*0.5-120));
       // okBtn->setScale(0.5);
        ...
        okTTF->setPosition(ccp(winSize.width*0.5,winSize.height*0.5-120));
        okTTF->setScale(0.7);
...
//    }
    if (curGold > 0) {
     ...
     CCLabelTTF*    jiacurGoldTTF = CCLabelTTF::create(TotalGoldChar, "Verdana-Bold", 45);
        jiacurGoldTTF->setPosition(ccp(winSize.width*0.5-50,winSize.height*0.5+50));
        jiacurGoldTTF->setScale(0.7);
        ...
        jiacurGoldTTF->runAction(CCSequence::create(CCMoveTo::create(1.0f, ccp(winSize.width*0.5-10,winSize.height-120)),CCCallFuncN::create(this, callfuncN_selector(FirstScene::jiafenFinsh)),NULL));
...
void FirstScene::initData(){
    ...
    backGroundMusic = CCUserDefault::sharedUserDefault()->getIntegerForKey("backGroundMusic", 1);
    soundMusic = CCUserDefault::sharedUserDefault()->getIntegerForKey("soundMusic", 1);
...
void FirstScene::initUI(){
    ...
    for (int i = 0; i < 8; i++) {
        ...
        heartImg->setPosition(ccp(40+60*i,winSize.height-44));
        heartImg->setScale(0.8);
        addChild(heartImg);
    }
    ...
    CCSprite* GoldImg = CCSprite::create("sekuai.png", CCRectMake(64*5+0.5, 64, 64, 64));
    GoldImg->setPosition(ccp(winSize.width*0.5-20,winSize.height-120));
    GoldImg->setAnchorPoint(ccp(1, 0.5));
   // GoldImg->setScale(0.4);
    ...
    GoldTTF->setPosition(ccp(winSize.width*0.5-10,winSize.height-120));
    GoldTTF->setScale(0.7);
    ...
    BestscoreImgTTF->setPosition(ccp(winSize.width*0.5-20,winSize.height-180));
    BestscoreImgTTF->setScale(0.7);
    BestscoreImgTTF->setAnchorPoint(ccp(1, 0.5));
    BestscoreImgTTF->setColor(ccc3(217, 109, 27));
    ...
    ScoreTTF->setPosition(ccp(winSize.width*0.5-10,winSize.height-180));
    ScoreTTF->setScale(0.7);
    ...
    facebookBtn->setScale(0.5);
    facebookBtn->setPosition(ccp(winSize.width*0.5-120, 80));
    
    addChild(_soundBg);
    CCMenuItemSprite *soundBgOn = CCMenuItemSprite::create( CCSprite::createWithTexture(_soundBg->getTexture(), CCRectMake(110, 110, 110, 110)), CCSprite::createWithTexture(_soundBg->getTexture(), CCRectMake(110, 0, 110, 110)));
    CCMenuItemSprite *soundBgOff = CCMenuItemSprite::create( CCSprite::createWithTexture(_soundBg->getTexture(), CCRectMake(110, 0, 110, 110)), CCSprite::createWithTexture(_soundBg->getTexture(), CCRectMake(110, 110, 110, 110)));
    ...
    pBgAudio->setPosition(ccp(winSize.width*0.5,80));
    pBgAudio->setScale(0.5);
    pBgAudio->setTag(2);
    
    CCMenuItemSprite *soundEfOn = CCMenuItemSprite::create( CCSprite::createWithTexture(_soundBg->getTexture(), CCRectMake(0, 110, 110, 110)), CCSprite::createWithTexture(_soundBg->getTexture(), CCRectMake(0, 0, 110, 110)));
    CCMenuItemSprite *soundEfOff = CCMenuItemSprite::create( CCSprite::createWithTexture(_soundBg->getTexture(), CCRectMake(0, 0, 110, 110)), CCSprite::createWithTexture(_soundBg->getTexture(), CCRectMake(0, 110, 110, 110)));
    if (soundMusic ==1) {
       // CCLog("efok");
    ...
    pEfAudio->setPosition(ccp(winSize.width*0.5+120,80));
    pEfAudio->setScale(0.5);
    pEfAudio->setTag(1);
    ...
    pGameTop->setPosition(ccp(winSize.width-64,80));
   // pGameTop->setScale(1);
    ...
    pPlay->setPosition(ccp(winSize.width*0.5,winSize.height*0.5-300));
    //pPlay->setScale(0.5);
    ...
    HearNumTTF->setPosition(ccp(winSize.width-180, winSize.height-44));
    HearNumTTF->setScale(0.9);
...
void FirstScene::reportScoreToGameCenter(int _BestScore){
    [[GameKitHelper sharedGameKitHelper] reportScore:_BestScore forCategory:@"PCHD_T"];
}
void FirstScene::showprogessBar(){
    ...
    progessBar->setPosition(ccp(winSize.width-100, winSize.height-44));
    //progessBar->setScale(0.4);
    this->addChild(progessBar);
    
    ct = CCProgressTimer::create(CCSprite::create("heartTimeBar.png"));
    ct->setPosition(ccp(winSize.width-100, winSize.height-44));
   ...
   // HeartTTF->setString(heartChar);
    ...
    HeartTTF->setPosition(ccp(winSize.width-100, winSize.height-44));
    HeartTTF->setScale(0.8);
...
void FirstScene::showHeart(int Heart){
    ...
    for (int i = 0; i < heartNumId; i++) {
        ...
        int posi = 60*(heartNumId-1)+40;
        heartImg->setPosition(ccp(posi-60*i,winSize.height-44));
        heartImg->setScale(0.8);
        heartImg->setTag(50);
...
void FirstScene::pPlayCallBack(){
    ...
    if (_heartNum >0) {
       ...
       // _heartArray = (CCArray*)_heartBatch->getChildren();
        ...
        heart->runAction(CCSequence::create(CCMoveTo::create(1.0f, ccp(winSize.width*0.5,winSize.height*0.5-300)),CCCallFuncN::create(this, callfuncN_selector(FirstScene::PlayScene)),NULL));
    }else{
        CCLog("need buy");
        ReqPurchase(1);
    }
