...
//    CC_SAFE_RETAIN(m_WaterTweenBatchNode);
   ...
   // m_LineLightBatchNode = CCParticleBatchNode::create("lineLight01.png");
    ...
    m_ExplodingRingBatchNode = CCParticleBatchNode::create("ExplodingRing.png");
    CC_SAFE_RETAIN(m_ExplodingRingBatchNode);
...
void ParticleSysLayer::LineLightPlay(int paopaoType,CCPoint location){
   // CCLog("line");
    CCParticleSystemQuad* mParticle = CCParticleSystemQuad::create("lineLight01.plist");
    mParticle->setPosition(location);
   // mParticle->setPosition(CCPointMake(160, 240));
    mParticle->setAutoRemoveOnFinish(true);
    mParticle->setDuration(0.2);
    if (paopaoType == 1) {
        mParticle->setPosVar(ccp(5, 60));
        
    }else{
        mParticle->setPosVar(ccp(60, 5));
    }
    m_LineLightBatchNode->addChild(mParticle);
}
void ParticleSysLayer::ExplodingRingPlay(cocos2d::CCPoint location){
    CCParticleSystemQuad* mParticle = CCParticleSystemQuad::create("ExplodingRing.plist");
    mParticle->setPosition(location);
    mParticle->setAutoRemoveOnFinish(true);
   // mParticle->setDuration(-1);
    m_ExplodingRingBatchNode->addChild(mParticle);
}
void ParticleSysLayer::ExplosionPlay(int paopaoType, cocos2d::CCPoint location){
    ...
    // mParticle->setDuration(-1);
//        if (paopaoType == 1) {
//        mParticle->setPosVar(ccp(5, 50));
//    }else{
//        mParticle->setPosVar(ccp(50, 5));
//    }
